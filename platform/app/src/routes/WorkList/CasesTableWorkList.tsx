...
import { APP_MARKETING_LANDING_PATH, openAppPathInNewTab, viewerModePathname } from '../../constants/viewerRoutes';
...
function CasesTableWorkList({
  ...
  const openStudy = React.useCallback(
    (study: StudyRecord) => {
      ...
      openAppPathInNewTab(
        viewerModePathname(String(validMode.routeName), dataPath || ''),
        query
      );
    },
  ...
  const openStudyPreferMode = React.useCallback(
    (study: StudyRecord, keywords: string[]) => {
      ...
      openAppPathInNewTab(
        viewerModePathname(String(byKeyword.routeName), dataPath || ''),
        query
      );
    },
    [appConfig.loadedModes, dataPath, location.search]
  ...
  const onRowClickToggleExpand = React.useCallback((record: CaseRow) => {
  ...
  const onAddCase = useCallback(() => {
    ...
    if (!DicomUploadComponent || !ds?.getConfig?.()?.dicomUploadEnabled) {
      show({
        ...
        content: () => (
          <div className="p-4 text-sm text-slate-700" dir="rtl">
            تفعيل رفع DICOM من إعداد مصدر البيانات (<code className="rounded bg-slate-100 px-1">dicomUploadEnabled</code>)
            وتأكد من تسجيل مكوّن الرفع في المنصة.
          </div>
        ),
      });
      return;
  ...
  const prependCaseRowActions = React.useCallback((record: CaseRow) => {
    ...
    () => [
      {
        ...
        render: (value, record) => {
          ...
          return (
            <div
              className="text-start font-medium text-[#0b1120] break-words"
              title={
                uid
                  ? `Study UID / كود الحالة:\n${uid}`
                  : 'Study UID / كود الحالة: غير متوفر'
              }
            >
              {name}
            </div>
      ...
      },
      {
        ...
        width: 200,
        render: value => (
          <div className="text-start break-words">{value ? String(value) : '-'}</div>
        ),
      },
      {
        key: 'modalities',
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-1 flex-col overflow-y-auto overflow-x-hidden bg-white text-[#0b1120]">
        ...
        onConfirm={async () => {
          ...
          if (!orthancRest) {
            servicesManager.services.uiNotificationService?.show({
              title: 'تعذر الحذف',
              message: 'مصدر البيانات لا يعرّف مسار REST لـ Orthanc (orthancRestRoot أو جذر DICOMweb).',
              type: 'error',
              duration: 6000,
            });
            return;
      ...
      <main
        className="cases-table-scope flex min-h-0 flex-1 flex-col gap-3 overflow-visible bg-white p-4"
        dir="rtl"
      >
        <style>
          {`
            .cases-table-scope button,
            .cases-table-scope [role="button"] {
              background-color: #f8fafc !important;
              border: 1px solid #cbd5e1 !important;
              color: #0b1120 !important;
            }

            .cases-table-scope button:hover,
            .cases-table-scope [role="button"]:hover {
              background-color: #eef2f7 !important;
              border-color: #0b1120 !important;
              color: #0b1120 !important;
            }

            .cases-table-scope button svg,
            .cases-table-scope [role="button"] svg {
              color: #0b1120 !important;
              stroke: #0b1120 !important;
            }

            .cases-table-scope h1,
            .cases-table-scope h2,
            .cases-table-scope h3,
            .cases-table-scope p,
            .cases-table-scope label,
            .cases-table-scope thead,
            .cases-table-scope th,
            .cases-table-scope th *,
            .cases-table-scope .text-muted-foreground {
              color: #0b1120 !important;
            }

            .cases-table-scope input,
            .cases-table-scope select {
              background-color: #ffffff !important;
              border-color: #cbd5e1 !important;
              color: #0b1120 !important;
            }

            .cases-table-scope input:hover,
            .cases-table-scope select:hover,
            .cases-table-scope input:focus,
            .cases-table-scope select:focus {
              border-color: #0b1120 !important;
            }

            .cases-table-scope input::placeholder {
              color: rgba(11, 17, 32, 0.55) !important;
            }

            .cases-table-scope thead,
            .cases-table-scope thead tr,
            .cases-table-scope thead th {
              border: 0 !important;
            ...
            .cases-table-scope thead button,
            .cases-table-scope thead [role="button"] {
              width: 100% !important;
            ...
            .cases-table-scope thead button:hover,
            .cases-table-scope thead [role="button"]:hover {
              background-color: transparent !important;
              border-color: transparent !important;
            }
            ...
            .cases-table-scope *::-webkit-scrollbar {
              width: 8px;
              height: 8px;
            }

            .cases-table-scope *::-webkit-scrollbar-track {
              background: transparent;
            }

            .cases-table-scope *::-webkit-scrollbar-thumb {
              background-color: #cbd5e1;
              border-radius: 999px;
              border: 2px solid #ffffff;
            }

            .cases-table-scope *::-webkit-scrollbar-thumb:hover {
              background-color: #94a3b8;
            }
          `}
        </style>
        <section
          className="flex shrink-0 flex-col gap-3 rounded-2xl border border-[#0b1120] px-4 py-3 shadow-sm md:flex-row md:items-center md:justify-between"
          style={{ borderColor: moduleColor, borderTop: `4px solid ${moduleColor}` }}
        >
          {/* قائمة الحالات */}
          <div>
            <h1 className="text-2xl font-bold text-[#0b1120]">قائمة الحالات</h1>
            <p className="mt-1 text-sm font-medium text-[#0b1120]">
              شاشة تشغيل يومية واضحة لعرض الحالات وفتح الدراسة مباشرة من الجدول
            </p>
          </div>
          {/* زر تحديث الحالات */}
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-sm font-medium text-[#0b1120]">
              النتائج: {summary.studies.toLocaleString('en-US')}
            </span>
            <button
              type="button"
              onClick={onRefresh ?? (() => {})}
              className="rounded-lg border border-slate-300 bg-slate-50 px-4 py-2 text-sm font-semibold text-[#0b1120] shadow-sm transition hover:border-[#0b1120] hover:bg-slate-100"
            >
              تحديث الحالات
            </button>
          </div>
        ...
        <section className="grid shrink-0 grid-cols-2 gap-3 lg:grid-cols-5">
          {[
            ['عدد الحالات', summary.studies],
            ['عدد المرضى', summary.patients],
            ['أنواع الفحوصات', summary.modalities],
            ['حالات اليوم', summary.todayStudies],
            ['لها وصف', summary.withDescription],
          ].map(([label, value]) => (
            <div
              key={label}
              className="rounded-xl border border-[#0b1120] bg-white p-3 shadow-sm"
            >
              <div className="text-xs font-semibold text-[#0b1120]">{label}</div>
              <div className="mt-1 text-xl font-bold text-[#0b1120]">
        ...
        <div className="flex w-full flex-col">
          <CtrDataTableAdvanced<CaseRow>
            ...
            // description="كل البيانات الأساسية مرتبة وقابلة للبحث والتصفية"
          dataSource={rows}
          columns={columns}
          ...
          activeRowClassName="!bg-sky-100 ring-2 ring-inset ring-sky-300/70"
...
export default CasesTableWorkList;
